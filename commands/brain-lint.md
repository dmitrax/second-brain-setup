...
| `taskboard-done:<project>` | `brain.sh archive` — moves dated Done entries into the archive note. Never retype entries by hand. The detail names how many of them are dated: when that number is far below the total, `archive` cannot clear the threshold and running it again will not help — the entries were closed without a date, and a top-level entry carries `YYYY-MM-DD` from the moment it is closed. The fix is not to date them retroactively from git (commit dates are not completion dates) but to nest the undated ones under the dated parent step they belong to; a closed sub-item is not an archivable entry and is not counted as one. |
