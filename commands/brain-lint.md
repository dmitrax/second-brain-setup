...
| `closed-outside-done:<project>` | Closed items sitting in a section that is not `## Done`, with the sections named. Nothing files them: `sweep-closed` walks `In progress` by construction — correct for its job, since that is the section the threshold measures — so an item ticked in `Backlog` stays there for good. Measured 2026-08-29: 52 on this project's own board, 28 in goprofi, 23 and 20 in the dimarch pair, against an estimate of seven. **Date them before moving them:** `brain.sh backfill-dates <taskboard>` reads history across the whole file, so an entry ticked while it was still in `In progress` is datable; move undated entries into `Done` and you create a threshold no amount of archiving can satisfy. A fact, never a threshold — the queue is not debt, and a size limit on `Backlog` is refused on the same grounds. |
