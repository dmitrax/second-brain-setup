...
`ambiguous-link`, `closed-outside-done`, `current-state`, `ffc-budget`, `key-uniformity`,
`retelling-no-source`,
`session-list`, `taskboard-done`, `taskboard-inprogress`, `taskboard-read`, `wiki-no-backlink`,
...
| `taskboard-inprogress:<project>` | Counted on the **largest single `###` block** of open top-level items, never on their sum, and the detail names that block. Two re-aims, each on evidence: from lines to items, because a board requiring each task to carry its measurement read 1148/300 with 64 genuinely open tasks; then from the sum to the block (2026-08-30), because the sum adds "how much is not cleared" to "how much is planned" and punishes a roadmap as debt. Measured on the two boards it had ever accused — the only sound test, since their plan share differs: 218 open items of which 17 are a sprint order and 14 a session plan, and neither recorded candidate brought it under budget (by priority marker 135, by dropping the plan sections 187). The largest block isolates what the analysis called real debt. `brain.sh sweep-closed` first: it moves closed top-level items out of In progress into Done, which is where the weight usually is. Then `archive` on what landed there, with both flags as above. If the block still exceeds, it is genuinely open work — say so and leave it as a task rather than trimming live content. |
| `taskboard-read:<project>` | Kilobytes above the first queue or record heading (`Backlog` in any case, `Бэклог`, `Done`, `Завершено`) — the part a session reads in full at every start; over 64 KB, about 19 thousand tokens. A **cost, not a debt**, and that is why it is not one more reading of `taskboard-inprogress`: every other taskboard counter asks how much work is not done, and `goprofi-voronka` grew to 14 087 lines (~395 thousand tokens) with all of them green — first in In progress blocks under 40 items, then in 144 sections moved below In progress, which emptied In progress and cost the reader nothing. Measured over every revision of all 15 boards on 2026-09-24: goprofi above 64 KB in 398 of 459, this project in 57 of 139 before its 09-15 clean-up, the other 13 never. What to do: move whatever is not current work below `## Backlog` (a queue is read by its headings only), `sweep-closed` and `archive` what is closed. Never delete live content to meet the number. |
| `closed-outside-done:<project>` | Closed items sitting in a section that is neither `## Done` nor `## In progress` — the two a tool files. The detail names each section with its size, largest first, and folds the tail after five (`+N more sections (M items)`), because one figure hides different answers: goprofi's 164 on 2026-09-10 were 98 in a session plan, ~28 in sections its owner keeps as done, and 19 in `Backlog`. Nothing files them: `sweep-closed` walks `In progress` by construction, so an item ticked in `Backlog` stays there for good. Measured 2026-08-29: 52 on this project's own board, 28 in goprofi, 23 and 20 in the dimarch pair, against an estimate of seven. **Move and date in one pass:** `brain.sh backfill-dates <taskboard>` dates entries in `Done` only, and reads history across the whole file, so an entry ticked outside `Done` is datable once it is there — move, then run it with `--apply` before the save. Left undated in `Done`, it creates a threshold no amount of archiving can satisfy. (Until 2026-09-15 this said "date them before moving them", an order the command could not carry out; it now names the undated entries it did not look at.) A fact, never a threshold — the queue is not debt, and a size limit on `Backlog` is refused on the same grounds. |
...
| `decision-schema/-ref/-legacy/-field:<path>` | Off-schema `status:`; a reference to a note that does not exist; the legacy one-line supersession form (invalid YAML — Obsidian drops that note from every property query); or a reference field carrying prose beside the note name, keyed `<path>#<field>`. A prose value is reported as `-field` and never as `-ref`: the target usually exists, so "does not exist" would send you to the wrong repair — put the qualifier in the new note's body, which is where a partial supersession belongs. Fix on sight. |
...
| `scope-note:lifecycle-docs` | Not a finding — the inventory of documents that have a state (briefs, audit requests, verification plans) with the state each carries. **Inside `wiki/` the trait is DECLARED by `type:` and never inferred**, because the two cheaper traits were killed by measurement: `status:` is carried by 56 ordinary synthesis notes, and `tags:` is no better — `audit` sits on five notes *about* audits as well as on five audit requests. A note declaring `type:` is a document with a process, not a wiki note, so the three link rules skip it: a backlink says nothing about an instruction that dies with its run, and a sibling link would be invented to reach a count. If a note is flagged by `wiki-no-backlink` and is really a brief, declare `type:` — never add the link. Verify each against the work, not against its age: a brief legitimately stays open for weeks, which is why there is no threshold here. A final one takes `closed: <date>` next to its status. |
