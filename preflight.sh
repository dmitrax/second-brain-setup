...
if [ "$scanned" -eq 0 ]; then
    ...
    fail "флаг, расходящийся между GNU и BSD, без фоллбека (shebang это не лечит)" "$missing"
else
    pass "непортируемые флаги coreutils отсутствуют или имеют фоллбек ($scanned файлов)"
fi
